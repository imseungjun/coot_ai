...
export function VideoLinkTile({
  ...
  /** oEmbed 채널명은 표시하지 않음 — 편집한 link.name과 섞여 보이지 않게 함 */
  const metaSecondary =
    provider === "youtube" || effectiveYtId
      ? "YouTube"
      : provider === "vimeo"
        ? "Vimeo"
        : hostLabel(link.url);
  ...
  return (
    ...
    >
        ...
        <div className="relative aspect-video w-full overflow-hidden rounded-xl bg-zinc-900 ring-1 ring-white/[0.06]">
          {effectiveYtId && ytThumb && !thumbDead ? (
            ...
            <div className="flex h-full w-full items-center justify-center bg-gradient-to-br from-zinc-800 to-zinc-950">
              <span
                className="h-10 w-10 animate-pulse rounded-full border-2 border-white/15 border-t-coot-accent"
                aria-hidden
              />
            </div>
            ...
            <div className="flex h-full w-full flex-col items-center justify-center gap-2 bg-gradient-to-b from-zinc-800 to-zinc-950 px-4">
              <span className="rounded bg-red-600 px-2 py-1 text-[10px] font-bold text-white">YouTube</span>
              <span className="text-center text-[10px] text-coot-muted">검색 링크 · 썸네일 없음</span>
            </div>
          ) : (
            <div className="flex h-full w-full flex-col items-center justify-center gap-2 bg-gradient-to-b from-zinc-800 to-zinc-950 px-4">
              {iconSrc && imgOk ? (
          ...
          {effectiveYtId || showVimeoOrRemote ? (
      ...
      <details className="absolute right-1 top-1 z-30">
        <summary
          className="list-none flex h-8 w-8 cursor-pointer items-center justify-center rounded-full bg-black/55 text-coot-muted opacity-95 ring-1 ring-white/10 backdrop-blur-sm transition-opacity hover:text-coot-text [&::-webkit-details-marker]:hidden"
          aria-label="링크 메뉴"
        >
          ⋮
        </summary>
